...
# Держим лазер ВНУТРИ прямоугольника цели. Улучшен детектор лазера + фикс. экспозиция.
...
class Settings:
    ...
    # Принудительные настройки камеры (важно для видимости лазера)
    AE_ENABLE = False          # выключим автоэкспозицию
    EXPOSURE_US = 2000         # 2 ms (под лазер обычно 1000–3000 мкс ок)
    ANALOG_GAIN = 1.5          # умеренный аналоговый гейн
...
class StateMachine:
    ...
    def update(self, obj_detected, dot_detected):
        ...
        if obj_detected and dot_detected:
            ...
            self.state = "CHASE" if self.miss_dot_counter < Settings.MISS_FRAMES_TO_SEARCH else "SEARCH"
        else:
            self.state = "IDLE"
            self.miss_dot_counter = 0
        if self.state != prev:
...
class TurretController:
    def __init__(self):
        ...
        self.last_dot = None  # для отладки/будущих улучшений
    ...
    def _init_camera(self):
        ...
        cam.start()
        # важные ручные настройки экспозиции/усиления
        try:
            cam.set_controls({
                "AeEnable": bool(Settings.AE_ENABLE),
                "AnalogueGain": float(Settings.ANALOG_GAIN),
                "ExposureTime": int(Settings.EXPOSURE_US)
            })
            print(f"[CAMERA] AE={Settings.AE_ENABLE}, Exp={Settings.EXPOSURE_US}us, Gain={Settings.ANALOG_GAIN}")
        except Exception as e:
            print("[CAMERA] set_controls warning:", e)
        print("[CAMERA] ok")
        return cam
    ...
    def _init_model(self):
        ...
        self.wanted_ids = [self.name2id[n] for n in Settings.WANTED_CLASSES if n in self.name2id] \
            if Settings.WANTED_CLASSES else []
        print(f"[MODEL] loaded: {Settings.WANTED_CLASSES or 'ALL'}")
        return model
    ...
    def send_angles(self, ax, ay):
        if self.s.PROTOCOL_MODE.upper() == "XY":
            ...
            yaw = ax - 90.0
            pit = ay - 90.0
            self._raw_send(f"ANG {yaw:.1f} {pit:.1f}")
    ...
    def point_to_rect_error(px, py, x1, y1, x2, y2, margin=4):
        """возвращает (dx,dy) до ближайшей точки прямоугольника с зазором; 0,0 если уже внутри"""
    ...
    def _select_target(self, results):
        """Возвращает (cx,cy,x1,y1,x2,y2) крупнейшего нужного класса или None"""
        if not results or len(results[0].boxes) == 0:
            return None
        best = None; best_area = 0
        for box in results[0].boxes:
            cls = int(box.cls.item())
            if self.wanted_ids and cls not in self.wanted_ids:
                continue
    ...
    def _detect_laser(self, frame):
        """Надёжный детектор: HSV + яркий красный в RGB + морфология."""
        # HSV красный
        ...
        # Яркий красный по RGB (R значительно > G и B)
        b, g, r = cv2.split(frame)
        # условия яркости и доминирования R
        ...
        a = cv2.contourArea(c)
        # фильтр площади (подстрой при необходимости)
        if a < 3 or a > 200:
            return None
        M = cv2.moments(c)
        if not M["m00"]:
        ...
        # защитимся от ложных бликов по краям
        if x < 6 or x > self.w-6 or y < 6 or y > self.h-6:
            return None
        return (x, y)
    ...
    def _handle_state(self, state, target, laser_pos):
        ...
        if state == "TRACK" and laser_pos:
            dx_px, dy_px = self.point_to_rect_error(laser_pos[0], laser_pos[1], x1, y1, x2, y2, margin=6)
            self._move_by_pixel_error(dx_px, dy_px)
        ...
        try:
            while True:
                ...
                if target:
                    ...
                    cv2.circle(frame, (tcx,tcy), 5, (255,0,0), 2)
                if laser_pos:
                    cv2.circle(frame, laser_pos, 8, (0,0,255), 2)
                cv2.drawMarker(frame, (self.cx,self.cy), (255,255,255), cv2.MARKER_CROSS, 20, 2)
                info = f"S:{self.state.state} Kp:{self.kp:.2f} invX:{int(self.invert_x)} invY:{int(self.invert_y)}  Dot:{'yes' if laser_pos else 'no'}"
                cv2.putText(frame, info, (10,25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,0), 2)

                cv2.imshow("Auto Aim (hold-in-box)", frame)
                key = cv2.waitKey(1) & 0xFF
                if key == ord('q'): break
                elif key == ord('l'): self.send_laser(!self.laser_on)
                elif key == ord('c'):
    ...
    def cleanup(self):
        ...
        if self.picam2:
            self.picam2.stop(); print("[CAMERA] остановлена")
